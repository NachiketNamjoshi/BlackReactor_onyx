 ...
 *
 *  Copyright (C)  2016 NachiketNamjoshi <nachiketnamjoshi@gmail.com>
 *
...
int cpufreq_reactive_cpu_lock(int num_core)
{
	int prev_lock;
...
int cpufreq_reactive_cpu_unlock(int num_core)
{
...
void cpufreq_reactive_min_cpu_lock(unsigned int num_core)
{
	int online, flag;
	struct cpu_dbs_info_s *dbs_info;
...
void cpufreq_reactive_min_cpu_unlock(void)
{
				  ...
				  const char *buf, size_t count)
{
	...
	if (ret != 1)
		return -EINVAL;
	if (input == 0)
		cpufreq_reactive_min_cpu_unlock();
	else
		cpufreq_reactive_min_cpu_lock(input);
	return count;
}
				  ...
				  const char *buf, size_t count)
{
	...
	if (prev_lock)
		cpufreq_reactive_cpu_unlock(prev_lock);
	...
	ret = cpufreq_reactive_cpu_lock(input);
	if (ret) {
...
static void cpufreq_reactive_early_suspend(struct early_suspend *h)
{
...
static void cpufreq_reactive_late_resume(struct early_suspend *h)
{
#if EARLYSUSPEND_HOTPLUGLOCK
...
static int __init cpufreq_gov_dbs_init(void)
{
	...
	dvfs_workqueue = create_workqueue("kreactive");
	if (!dvfs_workqueue) {
	...
	ret = cpufreq_register_governor(&cpufreq_gov_reactive);
	if (ret)
		goto err_reg;

#ifdef CONFIG_HAS_EARLYSUSPEND
	early_suspend.level = EARLY_SUSPEND_LEVEL_DISABLE_FB;
	early_suspend.suspend = cpufreq_reactive_early_suspend;
	early_suspend.resume = cpufreq_reactive_late_resume;
#endif
